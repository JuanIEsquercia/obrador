...
export default async function PaginaDetalleLicitacion({ params }: Props) {
  ...
  // Traer líneas y cotizaciones en paralelo
  const [{ data: lineas }, cotizacionesData] = await Promise.all([
    supabase
      .from('lineas_pedido')
      .select('*, familia:familias_producto(id, nombre)')
      .eq('pedido_id', licitacionId)
      .order('orden'),
  ...
  const cotizaciones = (cotizacionesData.data as CotizacionConLineas[]) ?? []
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-8 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="tarjeta flex items-start gap-3">
                <div className="w-10 h-10 rounded-xl bg-primary-fixed flex items-center justify-center flex-shrink-0 text-lg">📍</div>
              ...
              <div className="tarjeta flex items-start gap-3">
                <div className="w-10 h-10 rounded-xl bg-secondary-fixed flex items-center justify-center flex-shrink-0 text-lg">📅</div>
                <div>
            ...
            <div className="tarjeta">
              ...
              {familiasMap.size === 0 ? (
                <p className="text-sm text-on-surface-variant">Esta licitación no tiene ítems.</p>
              ) : (
                        ...
                        {ls.map(item => (
                          <div key={item.id} className="flex items-baseline justify-between gap-4 text-sm">
                            <div className="flex items-baseline gap-2 min-w-0">
                              ...
                              {item.notas && <span className="text-on-surface-variant text-xs truncate">({item.notas})</span>}
                            </div>
          ...
          <div className="lg:col-span-4">
            <AccionesPedido pedido={pedido as Pedido & { obra_id: string }} />
          </div>
        </div>
      </div>
    )
  ...
  const familiasArray = Array.from(
    new Map((lineas as LineaPedido[]).map(l => [l.familia_id, l.familia as FamiliaProducto])).entries()
  )
  ...
  return (
    <div className="max-w-5xl mx-auto">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-on-surface-variant mt-1">
              ...
              {pedido.fecha_cierre_cotizaciones && (
                <span>⏰ Cierre ofertas: {formatearFecha(pedido.fecha_cierre_cotizaciones)}</span>
              )}
              <span>📅 Entrega: {formatearFecha(pedido.fecha_entrega)}</span>
              {pedido.publicado_en && (
                <span>📢 Publicada: {formatearFecha(pedido.publicado_en)}</span>
              )}
            </div>
          </div>
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-on-surface">Cotizaciones recibidas</h2>
